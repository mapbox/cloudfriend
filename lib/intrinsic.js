 ...
 * @returns {object} The original string, in Base64 representation.
 */
const base64 = (value) => {
 ...
 * @returns {object} The value that is assigned to SecondLevelKey.
 */
const findInMap = (mapping, key, attr) => {
 ...
 * specifying region().
 * @returns {object} The list of Availability Zones for the region.
 */
const getAzs = (region) => {
 ...
 * final value.
 * @returns {object} The combined string.
 */
const join = (delimiter, pieces) => {
 ...
 * to dereference.
 * @returns {object} The physical ID of the resource or the value of the parameter.
 */
const ref = (name) => {
  return { Ref: name };
};
 ...
 * @returns {string} the array of commands, joined by `\n` and Base 64 encoded
 */
 ...
 * @returns {object} the value of an output exported by another stack.
 */
const importValue = (sharedValue) => {
 ...
 * @returns {object} a Fn::Sub object defining the ARN
 */
const arn = (service, suffix) => {
  const replacement = { service, suffix };
  if (service === 's3') return sub('arn:${AWS::Partition}:${service}:::${suffix}', replacement);
  else return sub('arn:${AWS::Partition}:${service}:${AWS::Region}:${AWS::AccountId}:${suffix}', replacement);
};

module.exports = { base64, findInMap, getAtt, getAzs, join, select, ref, userData, sub, importValue, split, arn };
