...
class Lambda {
  ...
  constructor(options = {}) {
    ...
    const MinimumEvaluationPeriods = Math.ceil(Timeout / Period);
    if (options.EvaluationPeriods < MinimumEvaluationPeriods)
      throw new Error('Cloudwatch alarm evalution window shorter than lambda timeout');

    const EvaluationPeriods = (options.EvaluationPeriods ?
